...
//**********************AGREGAR DATOS*******************************************************
			...
			parent::Conectar();
				...
				$valid_formats = array("xlsx", "xls" ,"doc", "docx", "pdf", "jpg", "png", "txt", "ppt", "pptx");
	        	 ...
	        	 move_uploaded_file($_FILES['archivo']['tmp_name'],"../docproyectos/docproye/".$_FILES["archivo"]["name"]);
			...
			//echo mysql_num_rows($result);exit;
			 ...
			 if (mysql_num_rows($result) == 0) {
			 	 ...
			 	 if ($_FILES['archivo']['size'] > $max_file_size) {
	             	header("Location: ../docproyectos.php?mensaje=6");
	             	continue;

	        	   } if (empty($_POST['idcategoria'])) {
 					 header("Location: ../docproyectos.php?mensaje=8");
 					  continue;

 					} if ($_POST['idcategoria']!=3) {
 					 header("Location: ../docproyectos.php?mensaje=9");
 					 continue;
								
		        	  } if( ! in_array(pathinfo($nom, PATHINFO_EXTENSION), $valid_formats) ){
		       		  header("Location: ../docproyectos.php?mensaje=7");
		       		  continue;
				...
				header("Location: ../docproyectos.php?mensaje=5");}

			 } else {
			 	header("Location: ../docproyectos.php?mensaje=4");
			 }
...
public function update($id){
			parent::Conectar();
				...
				$result = mysql_query($consulta);
				header("Location: ../docproyectos.php?mensaje=1");
...
//*************borrar archivo*************************************************************
				...
				public function delete($id){
							parent::Conectar();
								...
								header("Location: ../docproyectos.php?mensaje=2");
