...
//**********************AGREGAR DATOS*******************************************************
			...
			parent::Conectar();
				...
				$valid_formats = array("xlsx", "xls" ,"doc", "docx", "pdf", "jpg", "png", "txt", "ppt", "pptx");
	        	 ...
	        	 move_uploaded_file($_FILES['archivo']['tmp_name'],"../evainst/docevainst/".$_FILES["archivo"]["name"]);
			...
			//echo mysql_num_rows($result);exit;
			 ...
			 if (mysql_num_rows($result) == 0) {
			 	 ...
			 	 if ($_FILES['archivo']['size'] > $max_file_size) {
	             	header("Location: ../evainst.php?mensaje=6");
	             	continue;

	        	   } if (empty($_POST['idcategoria'])) {
 					 header("Location: ../evainst.php?mensaje=8");
 					  continue;

 					} if ($_POST['idcategoria']!=2) {
 					 header("Location: ../evainst.php?mensaje=9");
 					 continue;
								
		        	  } if( ! in_array(pathinfo($nom, PATHINFO_EXTENSION), $valid_formats) ){
		       		  header("Location: ../evainst.php?mensaje=7");
		       		  continue;
				...
				header("Location: ../evainst.php?mensaje=5");}

			 } else {
			 	header("Location: ../evainst.php?mensaje=4");
			 }
...
public function update($id){
			parent::Conectar();
				...
				$result = mysql_query($consulta);
				header("Location: ../evainst.php?mensaje=1");
...
//*************borrar archivo*************************************************************
				...
				public function delete($id){
							parent::Conectar();
								...
								header("Location: ../evainst.php?mensaje=2");
